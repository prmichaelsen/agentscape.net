// One-shot seed: encyclopedia entries #02 (tool), #06 (context-window), #16 (embedding).
// Adapted from seed-encyclopedia-05-12-13.mjs (same parser, same SQL shape).
...
const ENTRIES = [
  { slug: "tool",           title: "Tool",           entry_number: 2,  draft: `${DRAFT_BASE}/02-tool.md`,           related: ["agent","mcp","skill","permission"] },
  { slug: "context-window", title: "Context window", entry_number: 6,  draft: `${DRAFT_BASE}/06-context-window.md`, related: ["model","prompt","embedding","rag"] },
  { slug: "embedding",      title: "Embedding",      entry_number: 16, draft: `${DRAFT_BASE}/16-embedding.md`,      related: ["rag","model","prompt","context-window"] },
];
...
for (const e of ENTRIES) {
  ...
  const id = randomUUID();
  metas.push({ slug: e.slug, id, content_hash, published_at: now });
  ...
  // Reverse edges: existing mcp entry references tool/embedding; ensure forward edges from those back to mcp
  console.log(`SLUG ${e.slug} content_hash=${content_hash} id=${id}`);
}

writeFileSync("/tmp/seed-enc-02-06-16.sql", stmts.join("\n\n") + "\n");
writeFileSync("/tmp/seed-enc-02-06-16-meta.json", JSON.stringify(metas, null, 2));
console.log(`Wrote ${stmts.length} SQL stmts → /tmp/seed-enc-02-06-16.sql`);
