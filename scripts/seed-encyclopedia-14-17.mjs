// One-shot seed: encyclopedia entries #14 (MCP) and #17 (RAG).
// Parses the canonical 9-section drafts, computes content_hash, emits
...
const ENTRIES = [
  {
    ...
    title: "MCP",
    entry_number: 14,
    draft: `${DRAFT_BASE}/14-mcp.md`,
    related: [
      // section 6 of 14-mcp.md
      "agent",
      "tool",
      "skill",
      "permission",
  ...
  },
  {
    ...
    title: "RAG",
    entry_number: 17,
    draft: `${DRAFT_BASE}/17-rag.md`,
    related: [
      // section 6 of 17-rag.md
      "retrieval",
      "embedding",
      "memory",
      "prompt",
...
// Parse `## N. <Title>` ATX-2 sections out of the draft markdown.
// Returns { definition, first_appearance, etymology, current_meaning,
// divergent_meanings, supersession_note }. Citations footnote-block is
// appended to every body section so footnote refs resolve when the
// section renders standalone via renderMarkdown.
function parse9Section(path) {
  const raw = readFileSync(path, "utf8");
  // Strip leading frontmatter + scry skip comment + drafter notes tail.
  // Strategy: find every `## N. Title` heading and its body up to the
  // next `## ` or `---` divider.
  const lines = raw.split("\n");
  const sections = {};
  let i = 0;
  // First, isolate the citations footnote block so we can append it to
  // each section body (renderMarkdown sees footnotes locally).
  ...
  return {
    ...
    // §6 related, §7 citation, §8 amendment derived/auto-generated; not stored
    supersession_note: sections[9] && !/^Not applicable\.?/i.test(sections[9])
      ? sections[9]
      : null,
  };
}
...
for (const e of ENTRIES) {
  ...
  // entry_related: from_slug = this slug, to_slug = each related.
  // entry_related schema TBD — assume (id, from_slug, to_slug). Skip if
  // schema doesn't match; the section §6 list still renders inline in
  // the draft prose for now.
  ...
  // Print the hash for the callback payload.
  console.log(`SLUG ${e.slug} content_hash=${content_hash} id=${id}`);
}

writeFileSync(
  "/tmp/seed-enc-14-17.sql",
  stmts.join("\n\n") + "\n"
);
console.log(`Wrote ${stmts.length} SQL stmts → /tmp/seed-enc-14-17.sql`);
