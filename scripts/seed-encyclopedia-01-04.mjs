// One-shot seed: encyclopedia entries #01 (agent), #04 (harness).
// Adapted from seed-encyclopedia-02-06-16.mjs (same parser, same SQL shape).
...
const ENTRIES = [
  { slug: "agent",   title: "Agent",   entry_number: 1, draft: `${DRAFT_BASE}/01-agent.md`,   related: ["tool","loop","harness","scaffold"] },
  { slug: "harness", title: "Harness", entry_number: 4, draft: `${DRAFT_BASE}/04-harness.md`, related: ["agent","loop","scaffold","tool"] },
];
...
writeFileSync("/tmp/seed-enc-01-04.sql", stmts.join("\n\n") + "\n");
writeFileSync("/tmp/seed-enc-01-04-meta.json", JSON.stringify(metas, null, 2));
console.log(`Wrote ${stmts.length} SQL stmts → /tmp/seed-enc-01-04.sql`);
