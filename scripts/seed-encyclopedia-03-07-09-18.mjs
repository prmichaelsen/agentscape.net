// One-shot seed: encyclopedia entries #3 loop, #7 prompt, #9 scaffold, #18 retrieval.
...
const ENTRIES = [
  {
    ...
    title: "Loop",
    entry_number: 3,
    draft: `${DRAFT_BASE}/03-loop.md`,
    related: ["agent", "tool", "harness", "scaffold", "context-window"],
  },
  {
    slug: "prompt",
    title: "Prompt",
    entry_number: 7,
    draft: `${DRAFT_BASE}/07-prompt.md`,
    related: ["tool", "skill", "context-window", "eval", "rag", "retrieval"],
  },
  {
    slug: "scaffold",
    title: "Scaffold",
    entry_number: 9,
    draft: `${DRAFT_BASE}/09-scaffold.md`,
    related: ["harness", "agent", "loop", "tool", "skill"],
  },
  {
    slug: "retrieval",
    title: "Retrieval",
    entry_number: 18,
    draft: `${DRAFT_BASE}/18-retrieval.md`,
    related: ["rag", "embedding", "memory", "context-window"],
  },
];
...
function parse9Section(path) {
  ...
  return {
    ...
    supersession_note:
      sections[9] && !/^Not applicable\.?/i.test(sections[9])
        ? sections[9]
        : null,
  };
}
...
writeFileSync("/tmp/seed-enc-03-07-09-18.sql", stmts.join("\n\n") + "\n");
console.log(`Wrote ${stmts.length} SQL stmts → /tmp/seed-enc-03-07-09-18.sql`);
